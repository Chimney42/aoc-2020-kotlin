...
data class ActiveCube(val x: Int, val y: Int, val z: Int)
  ...
  File("data/day17-input.txt").bufferedReader().forEachLine {
    ...
    it.split("").slice(IntRange(1, 8)).forEach {
      if (it.toString().equals("#")) {
        activeCubes.add(ActiveCube(x, y, z))
      }
      x++
    }
    y++
    ...
    for (z in minZ-1 until maxZ+2) {
      for (y in minY-1 until maxY+2) {
        for (x in minX -1 until maxX+2) {
          val thisCube = activeCubes.find {it.x.equals(x) && it.y.equals(y) && it.z.equals(z)}
          val activeNeighbourCount = activeCubes
            //find all neighbours
            .filter {IntRange(x-1, x+1).contains(it.x) && IntRange(y-1, y+1).contains(it.y) && IntRange(z-1, z+1).contains(it.z)}
            .filterNot {it.x.equals(x) && it.y.equals(y) && it.z.equals(z)}
            .size
          if ((thisCube != null && IntRange(2, 3).contains(activeNeighbourCount)) || (thisCube == null && activeNeighbourCount == 3)) {
            activeCubesNextCycle.add(ActiveCube(x, y, z))
          }
        }
      }
    }
